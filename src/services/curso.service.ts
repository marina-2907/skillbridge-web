...
export const cursoService = {
  ...
  async listar(): Promise<CursoAPI[]> {
    return fetchAPI<CursoAPI[]>('/api/cursos', {
      method: 'GET',
    });
  },
  ...
  async buscarPorId(id: number): Promise<CursoAPI> {
    return fetchAPI<CursoAPI>(`/api/cursos/${id}`, {
      method: 'GET',
    });
  },
  ...
  async buscarPorArea(area: string): Promise<CursoAPI[]> {
    return fetchAPI<CursoAPI[]>(`/api/cursos/area/${area}`, {
      method: 'GET',
    });
  },
  ...
  async buscarPorNivel(nivel: string): Promise<CursoAPI[]> {
    return fetchAPI<CursoAPI[]>(`/api/cursos/nivel/${nivel}`, {
      method: 'GET',
    });
  },
  ...
  async pesquisar(nome: string): Promise<CursoAPI[]> {
    return fetchAPI<CursoAPI[]>(`/api/cursos/pesquisar?nome=${encodeURIComponent(nome)}`, {
      method: 'GET',
    });
  },
