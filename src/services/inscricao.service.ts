...
export const inscricaoService = {
  ...
  async listarPorUsuario(usuarioId: number): Promise<InscricaoAPI[]> {
    return fetchAPI<InscricaoAPI[]>(`/api/inscricoes/usuario/${usuarioId}`, {
      method: 'GET',
    });
  },
  ...
  async listarEmAndamento(usuarioId: number): Promise<InscricaoAPI[]> {
    return fetchAPI<InscricaoAPI[]>(
      `/api/inscricoes/usuario/${usuarioId}/em-andamento`,
      { method: 'GET' }
    );
  },
  ...
  async listarConcluidos(usuarioId: number): Promise<InscricaoAPI[]> {
    return fetchAPI<InscricaoAPI[]>(
      `/api/inscricoes/usuario/${usuarioId}/concluidos`,
      { method: 'GET' }
    );
  },
  ...
  async buscarPorId(id: number): Promise<InscricaoAPI> {
    return fetchAPI<InscricaoAPI>(`/api/inscricoes/${id}`, {
      method: 'GET',
    });
  },
  ...
  async criar(dados: InscricaoCreateDTO): Promise<InscricaoAPI> {
    return fetchAPI<InscricaoAPI>('/api/inscricoes', {
      method: 'POST',
  ...
  async atualizarProgresso(
    ...
    return fetchAPI<InscricaoAPI>(`/api/inscricoes/${id}/progresso`, {
      method: 'PUT',
      body: JSON.stringify(dados),
    });
  },
  ...
  async concluir(id: number, nota?: number): Promise<InscricaoAPI> {
    ...
    return fetchAPI<InscricaoAPI>(`/api/inscricoes/${id}/concluir`, {
      method: 'PUT',
      body: JSON.stringify(dados),
    });
  },
  ...
  async cancelar(id: number): Promise<void> {
    return fetchAPI<void>(`/api/inscricoes/${id}`, {
      method: 'DELETE',
    });
  },
};
